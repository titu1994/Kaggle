...
def fit_predict(data, labels, action, filename, test_datasets = [], learning_rate=0.3, n_epochs=1000, nkerns=[20, 50], batch_size=500, seed=8000):
...
def run():
    ...
    dtype = np.float32
    data = np.loadtxt(r"C:\Users\Yue\PycharmProjects\Kaggle\MNIST\Data\train.csv", dtype=dtype,
                           delimiter=',', skiprows=1)
    test_data = np.loadtxt(r"C:\Users\Yue\PycharmProjects\Kaggle\MNIST\Data\test.csv", dtype=dtype,
                          delimiter=',', skiprows=1)
    print (data.shape)
    ...
    subm[:, 1] = preds
    np.savetxt('mnist-lenet.csv', subm, fmt='%d', delimiter=',',
