...
Adapted for MNIST with "Transfer Learning"
'''
...
trainX = trainData[:, 1:].reshape(trainData.shape[0], 1, 28, 28)
...
# Entry module
x = ZeroPadding2D((2, 2), dim_ordering=DIM_ORDERING)(img_input)

x = conv2D_bn(x, 32, 3, 3, subsample=(2, 2), border_mode='valid')
...
x = MaxPooling2D((2, 2), strides=(2, 2), dim_ordering=DIM_ORDERING)(x)
...
#branch_pool = AveragePooling2D((3, 3), strides=(1, 1), border_mode='same', dim_ordering=DIM_ORDERING)(x)
#branch_pool = conv2D_bn(branch_pool, 192, 1, 1)
x = merge([branch1x1, branch7x7, branch7x7dbl, ], mode='concat', concat_axis=CONCAT_AXIS)
...
branch3x3 = conv2D_bn(branch3x3, 320, 3, 3, subsample=(1, 1), border_mode='same')

branch7x7x3 = conv2D_bn(x, 192, 1, 1)
branch7x7x3 = conv2D_bn(branch7x7x3, 192, 1, 7)
branch7x7x3 = conv2D_bn(branch7x7x3, 192, 7, 1)
branch7x7x3 = conv2D_bn(branch7x7x3, 192, 3, 3, subsample=(1, 1), border_mode='same')

#branch_pool = AveragePooling2D((3, 3), strides=(2, 2), dim_ordering=DIM_ORDERING)(x)
x = merge([branch3x3, branch7x7x3, ], mode='concat', concat_axis=CONCAT_AXIS)
...
#branch_pool = AveragePooling2D((3, 3), strides=(1, 1), border_mode='same', dim_ordering=DIM_ORDERING)(x)
#branch_pool = conv2D_bn(branch_pool, 192, 1, 1)
x = merge([branch1x1, branch3x3, branch3x3dbl, ], mode='concat', concat_axis=CONCAT_AXIS)
...
plot(model, to_file="Inception v3 MNIST.png", show_shapes=True)
...
model.load_weights("Inception Weights Best.h5")
print("Model loaded.")

#model.fit(trainX, trainY, batch_size=batch_size, nb_epoch=nb_epoch,
#          callbacks=[ModelCheckpoint("Inception Weights Best.h5", "val_acc", save_best_only=True)],
#          validation_data=(testX, testY))

model.save_weights("Inception Weights.h5", overwrite=True)
print("Model saved.")
...
print("Error Score : ", (100 - accuracy * 100), "%")
