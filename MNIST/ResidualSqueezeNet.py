...
def residual_block(input, nb_filter, nb_row, nb_col) -> Convolution2D:
    ...
    cn = Convolution2D(nb_filter, nb_row, nb_col, border_mode="same", activation="relu", init="glorot_uniform")(input)
    return cn
...
trainData = dc.convertPandasDataFrameToNumpyArray(dc.loadFullTrainData())
...
# Residual 1
fire2_expand3 = residual_block(fire2_squeeze, 64, 1, 3)
fire2_expand4 = residual_block(fire2_squeeze, 64, 3, 1)
...
# Residual 2
fire3_expand3 = residual_block(fire3_squeeze, 64, 1, 3)
fire3_expand4 = residual_block(fire3_squeeze, 64, 3, 1)
...
#Residual 3
fire4_expand3 = residual_block(fire4_squeeze, 128, 1, 3)
fire4_expand4 = residual_block(fire4_squeeze, 128, 3, 1)
...
# Residual 5
fire5_expand3 = residual_block(fire5_squeeze, 128, 1, 3)
fire5_expand4 = residual_block(fire5_squeeze, 128, 3, 1)
...
# Residual 6
fire6_expand3 = residual_block(fire6_squeeze, 192, 1, 3)
fire6_expand4 = residual_block(fire6_squeeze, 192, 3, 1)
...
# Residual 7
fire7_expand3 = residual_block(fire7_squeeze, 192, 1, 3)
fire7_expand4 = residual_block(fire7_squeeze, 192, 3, 1)
...
# Residual 8
fire8_expand3 = residual_block(fire8_squeeze, 256, 1, 3)
fire8_expand4 = residual_block(fire8_squeeze, 256, 3, 1)
...
# Residual 9
fire9_expand3  = residual_block(fire9_squeeze, 256, 1, 3)
fire9_expand4 = residual_block(fire9_squeeze, 256, 3, 1)
...
fire9_dropout = Dropout(0.25)(fire9)

#conv 10
conv10 = Convolution2D(128, 1, 1, init='glorot_uniform',border_mode='valid')(fire9_dropout)

#avgpool 1
#avgpool10 = AveragePooling2D((2,2))(conv10)
...
#model.load_weights("Residual Squeezenet Best.h5")
print("Model loaded")

testData = dc.convertPandasDataFrameToNumpyArray(dc.loadFullTestData())
testY = testData[:, 0]
testY = kutils.to_categorical(testY)
testX = testData[:, 1:]
testX = testX.reshape(testData.shape[0], 1, 28, 28)
testX = testX.astype(float)
testX /= 255.0

model.fit(trainX,trainY, batch_size=batch_size, nb_epoch=nb_epoch, validation_data=(testX, testY),
          callbacks=[ModelCheckpoint("Residual Squeezenet Best.h5", save_best_only=True)])
